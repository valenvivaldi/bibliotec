...
var controller = {
    all: (req, res) => {
         var books = Book.find().sort({uploadDate:-1}).limit(10).exec((err,data)=>{
             if (err) {
                return res.status(404).send({message:'error al solicitar todos los libros'});
             } else {
                ...
                .exec((err, doc) => {
                    ...
                    Book.insertMany(books, (err, docs) => {
                        if (err) {
                            ...
                            return res.status(200).send({
                                message: "carga de los libros del excel correcta!!!!!!! " +
                                    docs.length +
